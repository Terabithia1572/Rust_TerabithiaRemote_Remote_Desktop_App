...
hbb_name = 'terabithiaremote' + ('.exe' if windows else '')
...
def generate_control_file(version):
    ...
    content = """Package: terabithiaremote
Section: net
Priority: optional
Version: %s
Architecture: %s
Maintainer: terabithiaremote <info@rustdesk.com>
...
def build_flutter_deb(version, features):
    ...
    os.rename('rustdesk.deb', '../terabithiaremote-%s.deb' % version)
    os.chdir("..")
...
def build_flutter_windows(version, features, skip_portable_pack):
    ...
    system2(
        f'python3 ./generate.py -f ../../{flutter_build_dir_2} -o . -e ../../{flutter_build_dir_2}/terabithiaremote.exe')
    os.chdir('../..')
    if os.path.exists('./terabithiaremote_portable.exe'):
        os.replace('./target/release/terabithiaremote-portable-packer.exe',
                   './terabithiaremote_portable.exe')
    else:
        os.rename('./target/release/terabithiaremote-portable-packer.exe',
                  './terabithiaremote_portable.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/terabithiaremote_portable.exe')
    os.rename('./terabithiaremote_portable.exe', f'./terabithiaremote-{version}-install.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/terabithiaremote-{version}-install.exe')
...
def main():
    ...
    if windows:
        ...
        system2('mv target/release/terabithiaremote.exe target/release/TerabithiaRemote.exe')
        pa = os.environ.get('P')
        if pa:
            # https://certera.com/kb/tutorial-guide-for-safenet-authentication-client-for-code-signing/
            system2(
                f'signtool sign /a /v /p {pa} /debug /f .\\cert.pfx /t http://timestamp.digicert.com  '
                'target\\release\\terabithiaremote.exe')
        else:
            print('Not signed')
        system2(
            f'cp -rf target/release/TerabithiaRemote.exe {res_dir}')
        os.chdir('libs/portable')
        system2('pip3 install -r requirements.txt')
        system2(
            f'python3 ./generate.py -f ../../{res_dir} -o . -e ../../{res_dir}/terabithiaremote-{version}-win7-install.exe')
        system2('mv ../../{res_dir}/terabithiaremote-{version}-win7-install.exe ../..')
