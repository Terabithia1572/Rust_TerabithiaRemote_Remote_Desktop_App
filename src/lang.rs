...
pub fn translate_locale(name: String, locale: &str) -> String {
    ...
    let replace = |s: &&str| {
        ...
        if !crate::is_rustdesk() {
            if s.contains("TerabithiaRemote")
                && !name.starts_with("upgrade_terabithiaremote_server_pro")
                && name != "powered_by_me"
            {
                let app_name = crate::get_app_name();
                if !app_name.contains("TerabithiaRemote") {
                    s = s.replace("TerabithiaRemote", &app_name);
                } else {
